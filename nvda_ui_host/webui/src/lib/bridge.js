...
function applyPresentationState(payload, defaults = {}) {
	const controlsVisible = readPresentationValue(
		payload,
		"controls_visible",
	...
	const interactionMode = readPresentationValue(
		payload,
		"interaction_mode",
...
const DISPLAY_TOOLBAR_ACTIONS = new Set([
	"copy_text",
	"copy_markdown",
	"clear",
	"close",
...
function resolveDisplayPresentation(payload, { hasActions = false } = {}) {
	...
	const rawPresentation =
		payload?.display_presentation || metadata.display_presentation || {};
	const variant =
		typeof rawPresentation?.variant === "string" &&
		DISPLAY_VARIANTS.has(rawPresentation.variant)
	...
	const toolbarActions = Array.isArray(toolbar.actions)
		? toolbar.actions.filter(
				(action) =>
					typeof action === "string" && DISPLAY_TOOLBAR_ACTIONS.has(action),
			)
		: [];
	...
	return {
		...
		toolbarActions,
		toolbarPlacement:
			toolbar.placement === "after_content" ? "after_content" : "after_content",
	};
}
...
function updateConversationSummaries(payload) {
	const conversationSummaries = readPresentationValue(
		payload,
...
function replaceMessageById(message) {
	if (message?.id) {
		appState.chat.messages = appState.chat.messages.filter(
...
function updateControlState(payload) {
	const providerState =
		readPresentationValue(payload, "provider_state", {}) || {};
	const providerStatus =
		readPresentationValue(payload, "provider_status", {}) || {};
	const availableProviders = readPresentationValue(
		payload,
		"available_providers",
	);
	...
	if (providerStatus && typeof providerStatus === "object") {
		appState.control.providerStatus = {
			state:
				typeof providerStatus.state === "string"
					? providerStatus.state
					: "ready",
			reason:
				typeof providerStatus.reason === "string"
	...
	if (typeof chatEnabled === "boolean") {
		...
		typeof providerStatus.can_infer === "boolean"
	) {
...
function renderDisplay(payload) {
	...
	const thinkingSummary = readPresentationValue(
		payload,
		"thinking_summary",
		null,
	);
	const thinkingCollapsed =
		readPresentationValue(payload, "thinking_visible_by_default", true) ===
		false;
	const blocks = [];
	const normalizedActions = Array.isArray(actions) ? actions : [];
	const displayPresentation = resolveDisplayPresentation(payload, {
	...
	if (payload.output_html) {
		...
		blocks.push({
			...
			text:
				payload.output_text ||
				payload.message ||
				t("no_content", "No content available."),
		});
	}
...
function appendChatMessage(payload) {
	...
	const appendedUserMessage = messages.some(
...
function ensureStreamingMessage(payload) {
	...
	const existingMessage = appState.chat.messages.find(
...
function findStreamingMessage(messageId) {
	return (
		appState.chat.messages.find((message) => message.id === messageId) || null
	);
}
...
function beginChatStream(payload) {
	...
	if (!existingMessage) {
		...
		updateSingleChatMessage(messageId, (message) => ({
			...
			streamId,
			streamSequence:
				message.streamId === streamId ? (message.streamSequence ?? -1) : -1,
			streamAborted: false,
		}));
	}
...
function applyChatStreamDelta(payload) {
	...
	const incomingSequence = Number.isInteger(payload.sequence)
		? payload.sequence
		: null;
	const existingMessage = findStreamingMessage(messageId);
	if (
		!existingMessage ||
		existingMessage.streamId !== streamId ||
		existingMessage.streaming !== true
	) {
		return;
	}
	updateSingleChatMessage(messageId, (message) => {
		const currentSequence = Number.isInteger(message.streamSequence)
		...
		if (textBlockIndex >= 0) {
			const currentText =
				typeof contentBlocks[textBlockIndex]?.text === "string"
					? contentBlocks[textBlockIndex].text
					: "";
...
function endChatStream(payload) {
	...
	const streamId =
		typeof payload.stream_id === "string" && payload.stream_id.trim()
			? payload.stream_id.trim()
			: null;
	const finalSequence = Number.isInteger(payload.final_sequence)
...
function abortChatStream(payload) {
	...
	const streamId =
		typeof payload.stream_id === "string" && payload.stream_id.trim()
			? payload.stream_id.trim()
			: null;
	const lastSequence = Number.isInteger(payload.last_sequence)
...
function showError(payload) {
	const errorMessage =
		payload.error_message || t("no_content", "No content available.");
	const details =
...
function updateProgress(payload) {
	...
	const progressMessage =
		payload.message || t("progress_default_message", "Working...");
	showDisplayText(
		`${t("progress_prefix", "Progress")}: ${progressMessage}`,
		"content",
	);
...
export function handleHostEnvelope(envelope) {
	...
	if (envelope.version !== 2) {
		setStatus(
			t("unsupported_protocol_status", "Unsupported host protocol version."),
			true,
		);
	...
	if (envelope.type !== "command" || !envelope.command?.name) {
		setStatus(
			t("unknown_message_type_status", "Unknown host message type."),
			true,
		);
	...
	if (!commandHandler) {
		...
		resetDisplayState();
		showDisplayText(
			`${t("unhandled_command_prefix", "Unhandled command")}: ${envelope.command.name}`,
			"content",
		);
...
export function initializeWebViewBridge() {
	...
	const handleMessage = (event) => {
		...
		try {
			...
			setStatus(
				t("parse_host_message_failed_status", "Unable to parse host message."),
				true,
			);
		...
		try {
			...
			setStatus(
				t("apply_host_command_failed_status", "Unable to apply host command."),
				true,
			);
			console.error("WebView host message handling error", error, envelope);
			reportUiFailure(
				envelope?.correlation_id || envelope?.id || null,
				"handler_error",
			);
		}
	};
