	...
	nodes.forEach((node) => {
		...
		[...node.attributes].forEach((attribute) => {
			...
			if (
				...
				!isSafeUrl(name, attribute.value)
			) {
	...
	return normalizeContentBlocks(blocks)
		.map((block) => {
			...
			if (block.type === "image") {
				return String(
					block.alt ||
						t("image_attachment_notice", "[Image attachment included]"),
				).trim();
			}
...
export function formatRoleLabel(role) {
	const value = String(role || "assistant")
		.trim()
		.toLowerCase();
	...
	return value
		...
		: t("assistant_heading", "Assistant response");
}
...
export function hasRenderedTables(blocks) {
	return normalizeContentBlocks(blocks).some(
	...
	const matrix = rows
		.map((row) =>
			Array.from(row.querySelectorAll("th, td")).map((cell) =>
				escapeMarkdownCell(cell.textContent || ""),
			),
		)
