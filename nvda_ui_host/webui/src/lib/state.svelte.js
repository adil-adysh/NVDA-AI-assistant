...
export function mergeLocalizedStrings(payload) {
	...
	const nextStrings =
		payload?.localized_strings || metadata.localized_strings || {};
...
function getConversationById(conversationId) {
	return (
		appState.chat.conversations.find(
...
function evaluateConversationSelectionState() {
	...
	const selectedConversation =
		typeof appState.chat.conversationId === "string"
	...
	const hasMessages =
		Array.isArray(appState.chat.messages) && appState.chat.messages.length > 0;
...
export function setDisplayBlocks(blocks = [], actions = [], presentation = {}) {
	...
	appState.display.toolbarActions = Array.isArray(presentation.toolbarActions)
		? presentation.toolbarActions
		: [];
	appState.display.toolbarPlacement =
		presentation.toolbarPlacement || "after_content";
}
...
export function setConversationSummaries(conversations = []) {
	...
	appState.chat.conversations = normalizedConversations.filter(
		(conversation) => {
