...
_OPENAI_VISION_FAMILIES = (
    "chatgpt-4o", "gpt-4o", "gpt-4.1", "gpt-4.5", "gpt-5",
    "gpt-4-turbo", "gpt-4-vision-preview", "o1", "o3", "o4",
)
...
class OpenAICompatProvider(LLMProvider):
    ...
    def __init__(self, config: OpenAICompatConfig) -> None:
        if llm_client is None:
            raise LLMProviderError(
                "llm_client Rust extension is not installed. "
    ...
    def supports_image_description(self) -> bool:
        return self._detect_ollama() or self._model_supports_images(
            self._resolve_model()
        )
    ...
    def summarize(
        ...
        if stream_handler is not None:
            ...
            return SummaryResponse(
                text=text, model=model, provider=self.provider_name()
            )
    ...
    def describe_image(
        ...
        if self._detect_ollama():
            return self._describe_image_ollama(
                model, image_base64, prompt, stream_handler
            )

        return self._describe_image_openai_compat(
            model, image_base64, prompt, stream_handler
        )
    ...
    def _get_model_info_ollama_native(
        self, model_name: str
    ) -> ProviderModelInfo | None:
    ...
    def _normalize_ollama_model(self, data: dict[str, Any]) -> ProviderModelInfo:
        ...
        capabilities = {
            "completion", "chat", "streaming", "text_input", "text_output", "tools",
        }
    ...
    def _capabilities_for_model(self, model_id: str) -> ProviderModelInfo:
        ...
        if litert_def is None and (
            lowered.endswith(".litertlm")
            or "litert-community/" in lowered
        ):
            capabilities.add("tools")
        ...
        try:
            if stream_handler is not None:
                text, tool_calls, chunks = self._stream_chat(
                    model, payload_messages, stream_handler, tools=tool_defs,
                )
                return LLMResponse(
                    text=text,
                    model=model,
    ...
    def _generate_ollama_multimodal(
        ...
        if tools:
            payload["tools"] = [
                build_function_tool_definition(t) for t in tools
            ]
    ...
    def _merge_streamed_tool_calls(
        ...
        for item in payload:
            ...
            if isinstance(func.get("arguments"), str):
                tf["arguments"] = (
                    f"{tf.get('arguments', '')}{func['arguments']}"
                )
    ...
    def _convert_message(self, message: Message) -> dict[str, Any]:
        ...
        for part in message.parts:
            if part.type == "text" and part.text is not None:
                ...
                rendered = (
                    f"[tool call: {part.tool_name or 'unknown'} "
                    f"arguments={json.dumps(part.tool_args or {})}]"
                )
