...
class ModelManagerDialog(wx.Dialog):
    ...
    def _build_ui(self) -> None:
        ...
        self._provider_combo.Bind(
            wx.EVT_CHOICE, self._on_provider_change,
        )
        ...
        details_label = wx.StaticText(
            self, label=_("Selected model details:"),
        )
        ...
        self._show_disabled_cb = wx.CheckBox(
            self, label=_("Show disabled models"),
        )
    ...
    def _refresh_model_list(self) -> None:
        ...
        self._displayed_models = (
            recommended_ready + recommended_download
            + other_ready + other_download
        )

        active_id = self._provider.active_model_id
        self._active_id_clean = (
            active_id.strip() if isinstance(active_id, str) else None
        )
    ...
    def _add_models(
        self, models: list[ManagedModel], enabled_ids: set[str],
    ) -> None:
        for m in models:
    ...
    def _update_buttons(self) -> None:
        ...
        can_del = (
            self._provider.features.delete
            and model.state == ModelState.DOWNLOADED
        )
    ...
    def _on_double_click(self, event: wx.ListEvent) -> None:
        ...
        is_enabled = self._enabled_store.is_enabled(
            self._provider.provider_id, model.id,
        )
    ...
    def _toggle_enabled(self, model: ManagedModel, enabled: bool) -> None:
        self._enabled_store.set_enabled(
            self._provider.provider_id, model.id, enabled,
        )
        self._refresh_model_list()
