...
def run(cmd: list[str], cwd: str | None = None) -> subprocess.CompletedProcess:
	print(f"  $ {' '.join(cmd)}")
	# Callers inspect .returncode directly, so failures must not raise.
...
def main():
	...
	try:
		import litert_lm
		# Check that the native lib loaded (intentional introspection of
		# litert_lm's internal FFI state).
		lib = litert_lm._ffi._get_lib()  # pylint: disable=protected-access
		print(f"  Native library loaded: {lib}")
		print("  IMPORT SUCCESS")
	# Any exception at this boundary means the import/DLL load failed;
	# the script must report it and clean up rather than propagate.
		...
		except Exception as dll_err:  # pylint: disable=broad-exception-caught
